...
class ModuleViewer extends Module {
    ...
    public function Init($bLocal = false) {
        ...
        $this->sCacheDir = Config::Get('path.runtime.dir'); //$this->compiled_dir;

		
		
        $this->SetResponseHeader('X-Powered-By', 'Sugata Ru');
    ...
    /**
     * Создает и возвращает объект Fenom
     *
     * @return Fenom
     */
    public function CreateFenomObject() {

		//путь до папки шаблоно 
		$dir = F::File_NormPath(Config::Get('path.skins.dir') . '/' . $this->GetConfigSkin() . '/tpls');

		$fenom = new Fenom(new Fenom\Provider($dir)); // подрубаем $this->dir

 		//подключаем аддон
		//https://github.com/fenom-template/fenom/blob/master/docs/ru/ext/extend.md 
		$fenom = Fenom\Extra::factory($dir, Config::Get('path.runtime.dir'), Array());
		 
	 
		 
		//+ собственные функции
		$fenom->addFunctionSmart('rus_num', 'Fenom\Fun::rus_num');
		$fenom->addFunctionSmart('ru_date', 'Fenom\Fun::ru_date');
	    $fenom->addFunctionSmart('wgroup_add', 'Fenom\Fun::wgroup_add');
		$fenom->addFunctionSmart('img', 'Fenom\Fun::img');
		$fenom->addFunctionSmart('asset', 'Fenom\Fun::asset'); 
		$fenom->addFunctionSmart('include_once', 'Fenom\Fun::include_once');   
		
 
		
		
		$fenom->addFunctionSmart('ass', 'Fenom\Fun::ass'); 
		
		 
		//$fenom->setOptions(Fenom::FORCE_INCLUDE);// собирает шаблоны 0.27 сек
		$fenom->setOptions(Fenom::AUTO_RELOAD); // обновляем кешь если шаблон изменился 0.19 сек
		 //$fenom->setOptions(Fenom::FORCE_COMPILE); // всегда пересобирать 	1.4 сек	
		// без параметров контроля ~ 0.01
     ...
     * @return Smarty
     */
    public function GetFenomObject() {
    ...
    protected function _tplInit() {
        ...
        $this->oFenom = $this->CreateFenomObject();

	 
        // * Устанавливаем необходимые параметры для Smarty ! Больше нет Смарти и нет его настроек
       // $this->oFenom->compile_check = (bool)Config::Get('smarty.compile_check');
       // $this->oFenom->force_compile = (bool)Config::Get('smarty.force_compile');
        //$this->oFenom->merge_compiled_includes = (bool)Config::Get('smarty.merge_compiled_includes');

        // * Подавляем NOTICE ошибки - в этом вся прелесть смарти )
      //$this->oFenom->error_reporting = error_reporting() & ~E_NOTICE;
        ...
        $sCompilePath = F::File_NormPath(Config::Get('path.smarty.compiled')); //$this->compiled_dir;  

        F::File_CheckDir($sCompilePath);
        $this->oFenom->setCompileDir($sCompilePath);
    ...
    protected function _tplTemplateExists($sTemplate, $bException = false) {
        ...
        $sSkin = $this->GetConfigSkin();
         if (!$bResult && $bException) {
            ...
            $sErrorInfo = 'Template Dirs: ' . implode('; ', $this->getTemplateDir());
    ...
    protected function _tplCreateTemplate($sTemplate, $aVariables = null) {
        ...
        if ($aVariables && is_array($aVariables)) { 
           ...
           /*   self::$_renderCount++;
               ...
               self::$_inRender += 1;		
	 	//$oTemplate = $oFenom->display($sTemplate);
                self::$_inRender -= 1;
              self::$_renderTime += (microtime(true) - self::$_renderStart);
                self::$_renderStart = 0;
				
				print '-=-=-='.self::$_renderTime; */
 ...
 //   protected function _tplSetOptions($aOptions) {
    ...
    /*    self::$_renderOptionsStack[] = array(
            'caching'        => $this->oFenom->caching,
            'cache_lifetime' => $this->oFenom->cache_lifetime,
        );
        if (isset($aOptions['cache'])) {
            $this->oFenom->caching = Smarty::CACHING_LIFETIME_SAVED;
            if ($aOptions['cache'] === false) {
                // Отключаем кеширование
                $this->oFenom->cache_lifetime = 0;
            } elseif (isset($aOptions['cache']['time'])) {
                if ($aOptions['cache']['time'] == -1) {
                    // Задаем бессрочное кеширование
                    $this->oFenom->cache_lifetime = -1;
                } elseif ($aOptions['cache']['time']) {
                    // Задаем время кеширования
                    $this->oFenom->cache_lifetime = F::ToSeconds($aOptions['cache']['time']);
                } else {
                    // Отключаем кеширование
                    $this->oFenom->cache_lifetime = 0;
                }
   ...
   // }
    ...
    protected function _tplRestoreOptions() {
        ...
        if (self::$_renderOptionsStack) {
            ...
            if (isset($aOptions['caching'])) {
                $this->oFenom->caching = $aOptions['caching'];
            }
            if (isset($aOptions['cache_lifetime'])) {
                $this->oFenom->cache_lifetime = $aOptions['cache_lifetime'];
            }
        }
    }
    ...
    protected function _tplAssign($sName, $xValue) {
        ...
        $this->oFenom->assign($sName, $xValue);
    }
    ...
    public function GetLocalViewer() {
        ...
        $oFenom = $oViewerLocal->GetfenomObject();
		$evg = ''; //проверить 2 аргумента и функцию внизу  $oTpl->fetch();
        $oFenom->assign($oViewerLocal->getTemplateVars(),$evg);
   ...
   /* public function GetSmartyVersion() {
    ...
    public function VarAssign() {
       ...
       $this->Assign('_aRequest', $aRequest);
        ...
        foreach ($aPlugins as $sPlugin => $oPlugin) {
            ...
            if ($sDir) {
                $this->addTemplateDir(array($sDir . 'tpls/', $sDir), $oPlugin->GetName(false));
		...
		if (isset($r['_all_'])){
			...
			foreach ($r['_all_'] as $rr => $item){
				...
				$rrr = new $rr(array('name'=>'userfeedBlogs'));
				$rrr->Exec();
			}
		}
       ...
       if (E::ActivePlugin('ls')) {
        ...
        if ($this->CheckTheme($sSkinTheme)) {
            $this->oFenom->compile_id = $sSkinTheme;
        } 
		$sSkinTheme = 'default';
    ...
    public function Fetch($sTemplate, $aVars = array(), $aOptions = array()) {
        ...
        if ($sTemplatePath = $this->TemplateExists($sTemplate, true)) {   
		   ...
		   //при аджакс вызывается лист шаблон для загрузки далее постов...
            $oTpl = $this->_tplCreateTemplate($sTemplatePath); 
            ...
            if ($aVars) { 
                $oTpl = $oTpl->assignAll($aVars);
            }
			

			$sContent = $oTpl->fetch($sTemplatePath);
          /*  self::$_renderCount++;
            self::$_renderStart = microtime(true);
            self::$_inRender += 1;
...
print_r($this->aWidgets);*/
     ...
     * @return string|bool
     */
    public function FenomDefaultTemplateHandler($sType, $sName, &$sContent, &$iTimestamp, $oFenom) {
        /**
